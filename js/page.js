((global, factory) => {
    ...
    return {
        init: () => {
            if (document.querySelector('#_tips_area')) {
                document.querySelector('#_tips_area').remove()
            }
            document.body.innerHTML += TIPS_AREA
            document.title = backend.config.site_name
        },
        loadtips: function() {
        ...
        showPageTips: (selector, message) => {
            let page_tips = document.querySelector(selector)
            page_tips.innerHTML = message
            page_tips.style.display = 'block'
        },
        normalTrigger: (callback) => {
            if(!document._tigger) {
                document._tigger = true
                callback()
                let timer = setTimeout(() => {
                    clearTimeout(timer)
                    document._tigger = false
                }, 2000)
            }
        },
        fetchAvatar: (url, imgId) => {
            try {
