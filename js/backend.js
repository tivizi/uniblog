( function( global, factory ) {
    ...
    function fetchArticlesToStorage() {
        return new Promise((resolve, reject) => {
            ...
            if(articles_raw) {
                ...
                let druation = new Date().getTime() - articles.uptime
                console.log('Cache druation: ', druation/1000, 's')
                if(druation/1000 < RAW_DATA_VALID_SECONDS) {
                    resolve(articles.rawData)
                    return
                }
                articles_list = articles.rawData
            }
            fetch(new Request('https://api.github.com/repos/' + USER_REPO + '/issues', {
                method: 'GET',
                headers: {
            ...
            then(resp => {
                if(resp.status == 304) {
                    localStorage.setItem(RAW_DATA_KEY, JSON.stringify({
                        uptime: new Date().getTime(),
                        rawData: articles_list
                    }))
    ...
    function renderArticleFromStorage(container_selector) {
        if(!container) throw 'container is required!'
        loadtips()
        let articles_raw = localStorage.getItem(RAW_DATA_KEY)
        if(articles_raw) {
            let articles = JSON.parse(articles_raw)
            renderArticlesFromArticleList(articles.rawData, container_selector)
            closetips()
        } else {
            closetips()
    ...
    function renderArticlesFromArticleList(articles, container_selector) {
        ...
        for(let i in json) {
            ...
            reply.href = '/articles#' + json[i].number
            ...
            author.innerHTML += new Date(Date.parse(json[i].created_at)).toLocaleDateString()
            ...
            if (content.innerHTML.length == 0) {
                let emptyContentDom = document.createElement('div')
                emptyContentDom.setAttribute('class', 'empty-content')
                emptyContentDom.innerHTML = '暂无内容（:）'
                content.appendChild(emptyContentDom)
            }
            ...
            fetchAvatar(json[i].user.avatar_url, avatarImg.id)
        }
    }
